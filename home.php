<?php include "header.php"; ?>

<header class="home-banner" role="banner">
	<div class="overlay"></div>
	<div class="row h-100">
		<div class="col-md-12 my-auto text-center">	
			<h1>
				WELCOME TO <b>ONCLICKBIZ</b>
			</h1>
			<h2>
				Your Search for best Service Ends here
			</h2>
			<div class="row">
				<div class="col-lg-12 search-btn-colm">
					<form class="form-inline my-2 my-lg-0">

						<input class="form-control mr-sm-2 search-input" type="search" placeholder="Enter Services" aria-label="Search">

						<button class="btn btn-primary btn-post-job btn-home-search my-2 my-sm-0" type="submit"><i class="fas fa-search"></i>Search</button>									
					</form>
				</div>						
			</div>				
		</div>
	</div>
</header>

<!-- section one icons -->
<div class="container-fluid section-one">
	<div class="row section-one-row">
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-taxi"></i>				
				</div>			
				<p>Cabs</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-tv"></i>				
				</div>			
				<p>Electronics</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="far fa-building"></i>
				</div>			
				<p>Electronics</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-fighter-jet"></i>				
				</div>			
				<p>Flight</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-briefcase"></i>				
				</div>			
				<p>Jobs</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-subway"></i>				
				</div>			
				<p>Train</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-umbrella"></i>				
				</div>			
				<p>Insurance</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-hotel"></i>				
				</div>			
				<p>Hotel</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-graduation-cap"></i>				
				</div>			
				<p>Education</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-dumbbell"></i>				
				</div>			
				<p>Fitness</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-door-open"></i>				
				</div>			
				<p>Real Estate</p>
			</a>
		</div>
		<div class="col-12 col-sm-4 col-md-3 col-lg-2 section-one-colm">
			<a href="#">
				<div class="fa-icons">
					<i class="fas fa-briefcase-medical"></i>				
				</div>			
				<p>Doctor</p>
			</a>
		</div>

		<div class="col-12 categories-link-colm">
			<a href="#">
				<p>
					View A to Z Categories <i class="fas fa-arrow-right"></i>
				</p>
			</a>
		</div>

	</div>
</div>

<!-- section two cards -->

<div class="container section-two">
	<div class="row section-two-title text-center">
		<h1>Trending Services</h1>
	</div>
	<div class="row section-two-row">
		<div class="col-12 col-md-6 col-lg-4 section-two-colm wow pulse">
			<div class="card">
				<div class="heart-icon">
					<a href="#"><i class="far fa-heart"></i></a>
				</div>
				<img class="card-img-top" src="img/home/card-hotel.jpg" alt="services">
				<div class="card-body">
					<a href="#"><h5 class="card-title">Best Hotel Room</h5></a>
					<h3 class="card-last-update">Last update: 1 hr ago</h3>
					
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-map-marker-alt"></i>Kurla, Mumbai
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-clock"></i>Feb 14, 2018
							</a>
						</div>
					</div>
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-user"></i>Satish Sharma
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-folder"></i>Verified
							</a>
						</div>
					</div>
				</div>
			</div>		
		</div>

		<div class="col-12 col-md-6 col-lg-4 section-two-colm wow pulse">
			<div class="card">
				<div class="heart-icon">
					<a href="#"><i class="far fa-heart"></i></a>
				</div>
				<img class="card-img-top" src="img/home/card-cab.jpg" alt="services">
				<div class="card-body">
					<a href="#"><h5 class="card-title">Go Cab</h5></a>
					<h3 class="card-last-update">Last update: 1 hr ago</h3>
					
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-map-marker-alt"></i>Kurla, Mumbai
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-clock"></i>Feb 14, 2018
							</a>
						</div>
					</div>
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-user"></i>Satish Sharma
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-folder"></i>Verified
							</a>
						</div>
					</div>
				</div>
			</div>		
		</div>

		<div class="col-12 col-md-6 col-lg-4 section-two-colm wow pulse">
			<div class="card">
				<div class="heart-icon">
					<a href="#"><i class="far fa-heart"></i></a>
				</div>
				<img class="card-img-top" src="img/home/card-teaching.jpg" alt="services">
				<div class="card-body">
					<a href="#"><h5 class="card-title">Teaching Typing</h5></a>
					<h3 class="card-last-update">Last update: 1 hr ago</h3>
					
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-map-marker-alt"></i>Kurla, Mumbai
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-clock"></i>Feb 14, 2018
							</a>
						</div>
					</div>
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-user"></i>Satish Sharma
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-folder"></i>Verified
							</a>
						</div>
					</div>
				</div>
			</div>		  
		</div>
	</div>
</div>


<div class="container section-two">
	<div class="row section-two-title text-center">
		<h1>Featured Services</h1>
	</div>
	<div class="row section-two-row">
		<div class="col-12 col-md-6 col-lg-4 section-two-colm wow pulse">
			<div class="card">
				<div class="heart-icon">
					<a href="#"><i class="far fa-heart"></i></a>
				</div>
				<img class="card-img-top" src="img/home/card-world.jpg" alt="services">
				<div class="card-body">
					<a href="#"><h5 class="card-title">Travel World</h5></a>
					<h3 class="card-last-update">Last update: 1 hr ago</h3>
					
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-map-marker-alt"></i>Kurla, Mumbai
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-clock"></i>Feb 14, 2018
							</a>
						</div>
					</div>
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-user"></i>Satish Sharma
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-folder"></i>Verified
							</a>
						</div>
					</div>
				</div>
			</div>		
		</div>

		<div class="col-12 col-md-6 col-lg-4 section-two-colm wow pulse">
			<div class="card">
				<div class="heart-icon">
					<a href="#"><i class="far fa-heart"></i></a>
				</div>
				<img class="card-img-top" src="img/home/card-clinic.jpg" alt="services">
				<div class="card-body">
					<a href="#"><h5 class="card-title">Royal Clinic</h5></a>
					<h3 class="card-last-update">Last update: 1 hr ago</h3>
					
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-map-marker-alt"></i>Kurla, Mumbai
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-clock"></i>Feb 14, 2018
							</a>
						</div>
					</div>
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-user"></i>Satish Sharma
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-folder"></i>Verified
							</a>
						</div>
					</div>
				</div>
			</div>		
		</div>

		<div class="col-12 col-md-6 col-lg-4 section-two-colm wow pulse">
			<div class="card">
				<div class="heart-icon">
					<a href="#"><i class="far fa-heart"></i></a>
				</div>
				<img class="card-img-top" src="img/home/card-gym.jpg" alt="services">
				<div class="card-body">
					<a href="#"><h5 class="card-title">Golds Gym</h5></a>
					<h3 class="card-last-update">Last update: 1 hr ago</h3>
					
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-map-marker-alt"></i>Kurla, Mumbai
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-clock"></i>Feb 14, 2018
							</a>
						</div>
					</div>
					<div class="row card-text-row">
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-user"></i>Satish Sharma
							</a>
						</div>
						<div class="col-12 col-md-6">
							<a href="#">
								<i class="fas fa-folder"></i>Verified
							</a>
						</div>
					</div>
				</div>
			</div>		
		</div>
	</div>
</div>

<!-- section three black bg row -->

<div class="container-fluid section-three">
	<div class="row section-three-row">
		<div class="col-md-3 text-center">
			<i class="fas fa-folder-open"></i>
			<h1>12090</h1>
			<p>Regular Service</p>
		</div>
		<div class="col-md-3 text-center">
			<i class="fas fa-map-marker-alt"></i>
			<h1>960</h1>
			<p>Locations</p>
		</div>
		<div class="col-md-3 text-center">
			<i class="fas fa-user"></i>
			<h1>234562</h1>
			<p>Regular Members</p>
		</div>
		<div class="col-md-3 text-center">
			<i class="fas fa-briefcase"></i>
			<h1>250</h1>
			<p>Permium Ads</p>
		</div>
	</div>
</div>


<!-- section four cards -->
<div class="container-fluid section-four-container-fluid">
	<div class="container section-four-container">
		<div class="row section-four-row">
			<div class="col-md-4 wow zoomIn">
				<div class="card">
					<div class="card-body text-center">
						<i class="fas fa-certificate"></i>
						<h5 class="card-title">Best Services</h5>
						<p class="card-text">Lorem ipsum dolor sit amet, consectetur adipisicing elit, eiusmod
							tempor incididunt ut labore.</p>
					</div>
				</div>
			</div>			
			<div class="col-md-4 wow zoomIn">
				<div class="card">
					<div class="card-body text-center">
						<i class="fas fa-paint-brush"></i>
						<h5 class="card-title">Best Services</h5>
						<p class="card-text">Lorem ipsum dolor sit amet,consectetur adipisicing elit, eiusmod
						tempor incididunt ut labore.</p>
					</div>
				</div>
			</div>
			<div class="col-md-4 wow zoomIn">
				<div class="card">
					<div class="card-body text-center">
						<i class="fas fa-headset"></i>
						<h5 class="card-title">Best Services</h5>
						<p class="card-text">Lorem ipsum dolor sit amet,consectetur adipisicing elit, eiusmod
						tempor incididunt ut labore.</p>
					</div>
				</div>
			</div>			
		</div>			
	</div>
</div>

<!-- section five subscribe -->
<div class="containe-fluid section-five">
	<div class="row section-five-row">
		<div class="col-md-6">
			<h1>Join our 10,000+ subscribers and get access to the latest templates, freebies, announcements and resources!</h1>
		</div>
		<div class="col-md-6 search-btn-colm email-sub-section-five">
					<form class="form-inline my-2 my-lg-0">

						<input class="form-control mr-sm-2 search-input" type="search" placeholder="Enter your email" aria-label="Search">

						<button class="btn btn-primary btn-post-job btn-home-search my-2 my-sm-0" type="submit">Subscribe</button>									
					</form>
			</div>						
	</div>
</div>

<?php include "footer.php"; ?>